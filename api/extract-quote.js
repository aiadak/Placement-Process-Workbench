// Vercel Serverless Function - POST /api/extract-quote
// Extracts structured reinsurance quote fields from a raw email using OpenAI.
// OPENAI_API_KEY is read server-side and NEVER exposed to the browser.
// If APP_PASSWORD is set, callers must send a matching x-app-password header,
// so this cost-incurring endpoint is protected even if the UI gate is bypassed.
//
// Vercel env vars: OPENAI_API_KEY (required), APP_PASSWORD (optional), OPENAI_MODEL (optional)
...
module.exports = async function handler(req, res) {
  ...
  if (expectedPw) {
    ...
    if (given !== expectedPw) return res.status(401).json({ error: "Unauthorized" });
  }
  ...
  if (!email || email.length < 10) return res.status(400).json({ error: "Provide an 'email' string in the request body." });
  ...
  try {
    ...
    if (!resp.ok) { const detail = await resp.text(); return res.status(502).json({ error: "OpenAI request failed", status: resp.status, detail: detail.slice(0, 500) }); }
